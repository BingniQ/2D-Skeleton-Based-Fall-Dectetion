...
class PartAwareLSTMCell(RNNCell):
    ...
    def call(self, skel_inputs, state):
        ...
        o_all_skel = _linear([body_list[0], body_list[1], body_list[2], body_list[3], body_list[4], h],
                             5 * self._num_units, True)
        o_all_skel = sigmoid(o_all_skel)
        new_c_list = []

        for ind, each_part in enumerate(body_list):
            concat_p = _linear([each_part, h], 3 * self._num_units, weight_name='weight_%d' % ind,
...
def _linear(args, output_size, bias, weight_name=_WEIGHTS_VARIABLE_NAME, bias_name=_BIAS_VARIABLE_NAME, bias_initializer=None,
    ...
    with vs.variable_scope(scope) as outer_scope:
        ...
        with vs.variable_scope(outer_scope) as inner_scope:
            ...
            biases = vs.get_variable(bias_name, [output_size], dtype=dtype, initializer=bias_initializer)
