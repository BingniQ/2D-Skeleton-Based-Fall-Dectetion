...
import PLSTM.PartAwareLSTMCell
import tensorflow as tf
import pandas as pd
from PLSTM.Features_PLSTM import batch_size, g_feat_dim, g_num_action, g_T_size, lr, n_layers
...
class PLSTM(object):
    ...
    def __init__(self, batch_size=batch_size, lr=lr, mode='train', n_layers=n_layers, time_step=g_T_size):
    ...
    def _build_graph(self):
        def _get_PLSTM_cells(hidden_layer):
            return PLSTM.PartAwareLSTMCell.PartAwareLSTMCell(num_units=hidden_layer)
        ...
        self.action_cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=output_logit, labels=self.action_gt)
    ...
    def generate_report(self, data, path):
        ...
        data_df.to_excel(writer, 'page_2', float_format='%.5f')
        writer.save()
